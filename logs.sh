...
rur=$(echo -e "$ambillae" | grep -Po '(?<=rur=)[^;]*')
mid=$(echo -e "$ambillae" | grep -Po '(?<=mid=)[^;]*')
csrf=$(echo -e "$ambillae" | grep -Po '(?<=csrftoken=)[^;]*')
mcd=$(echo -e "$ambillae" | grep -Po '(?<=mcd=)[^;]*')
rolout=$(echo -e "$ambilcuk" | grep -Po '(?<=rollout_hash":")[^"]*')
...
function aexzzzz(){
           ...
           if [[ $status =~ "ok" ]]; then
                 printf "${hijau}[#]${normal}SUCCES FOLLOWED LAE..]${hijau} \n"
                 exit
            elif [[ $status =~ "Please wait a few minutes" ]];then
             printf "${merah}[+]${normal}Tidak bisa DIIKUTI \n"
             sleep 30
        else
             printf "${merah}[-]${normal}Tidak Dapat DIIKUTI Menggunakan Alat Ini LAE... \n"
             fi
}
